...
import { Upload, Trash2, Link, CheckCircle, Clock, Play, Square } from 'lucide-react';
...
export default function VideoManager() {
  ...
  const handleAddToPlaylist = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPlaylist(data.video.youtubeUrls.map((url: string, i: number) => ({
          url,
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.remainingCount === 0) {
          setVideoStatus(prev => prev ? { ...prev, activeSource: null, youtubeUrls: [] } : null);
        }
  ...
  const handlePlayVideo = async (index: number) => {
    try {
      ...
      if (response.ok) {
        ...
        setVideoStatus(prev => prev ? { ...prev, activeSource: 'youtube' } : null);
  ...
  const handleUploadVideo = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`✅ Video berhasil diupload! Total: ${uploadedVideos.length + 1} video`);
      } else {
        const error = await response.json();
  ...
  const handleDeleteUploadedVideo = async (index: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.remainingCount === 0) {
          setVideoStatus(prev => prev ? { ...prev, activeSource: null } : null);
        }
  ...
  const handlePlayUploadedVideo = async (index: number) => {
    try {
      ...
      if (response.ok) {
        ...
        setVideoStatus(prev => prev ? { ...prev, activeSource: 'upload' } : null);
  ...
  const handlePlaySource = async (source: 'upload' | 'youtube') => {
    try {
      ...
      if (response.ok) {
        setVideoStatus(prev => prev ? { ...prev, activeSource: source } : null);
        alert(`✅ ${source === 'upload' ? 'Video uploaded' : 'YouTube playlist'} sekarang diputar di TV`);
      }
    } catch (error) {
    ...
    const fetchVideoStatus = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.video?.type === 'youtube' && data.video?.youtubeUrls?.length > 0) {
            setPlaylist(data.video.youtubeUrls.map((url: string, i: number) => ({
              url,
              videoId: extractVideoId(url),
              index: i,
          ...
          } else if (data.video?.type === 'upload' && data.video?.uploadedVideos?.length > 0) {
            setUploadedVideos(data.video.uploadedVideos.map((video: any) => ({
              url: video.url,
  ...
  return (
    <Card className="bg-gradient-to-br from-slate-900/80 to-slate-800/80 border border-slate-700 shadow-xl">
      <CardHeader>
        ...
        <p className="text-sm text-slate-400">Kelola video YouTube untuk ditampilkan di TV ruang tunggu</p>
      </CardHeader>
            ...
            <div className="bg-gradient-to-br from-red-500/10 to-red-600/10 border border-red-500/30 rounded-xl p-6">
              ...
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 rounded-lg bg-slate-800 border border-slate-600 text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
                  onKeyPress={(e) => e.key === 'Enter' && handleAddToPlaylist()}
                />
                <Button
                  onClick={handleAddToPlaylist}
                  disabled={uploading || !youtubeUrl.trim()}
                  className="bg-red-500 hover:bg-red-600 text-white px-6 disabled:opacity-50"
                >
                  {uploading ? (
              ...
              <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    {videoStatus?.activeSource !== 'youtube' && playlist.length > 0 && (
                      <Button
                  ...
                  {playlist.map((item, index) => (
                    <div
                      ...
                      className={`flex items-center gap-4 p-4 rounded-lg border transition-all ${
                        index === currentIndex && videoStatus?.activeSource === 'youtube'
                ...
                <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                  <p className="text-xs text-blue-300">
                    💡 Video akan diputar otomatis secara bergantian. Setelah video terakhir selesai, playlist akan mengulang dari awal.
                  </p>
                </div>
              </div>
            )}
            ...
            {playlist.length === 0 && (
              <div className="text-center py-12 bg-slate-800/30 rounded-xl border border-dashed border-slate-600">
                ...
                <p className="text-slate-500 text-sm">
                  Tambahkan URL YouTube di atas untuk membuat playlist
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            {uploadedVideos.length > 0 && (
              <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                ...
                <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                  <p className="text-xs text-blue-300">
                    💡 Klik tombol Play untuk memutar video di TV. Hanya satu video yang bisa diputar pada satu waktu.
                  </p>
                </div>
              </div>
            )}
